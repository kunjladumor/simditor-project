(function (root, factory) {
  ...
  SimditorMention = (function (superClass) {
    ...
    SimditorMention.prototype._bind = function () {
      ...
      this.editor.body.on(
        ...
        (function (_this) {
          return function (e) {
            ...
            $target = $('<span class="simditor-mention edit" />').append(
              $link.contents()
            );
      ...
      return this.editor.wrapper.on(
        "mousedown.simditor-mention",
        (function (_this) {
          return function (e) {
            if (
              $(e.target).closest(
    ...
    SimditorMention.prototype._renderPopover = function () {
      var $itemEl, $itemsEl, abbr, item, j, len, name, ref;
      this.popoverEl = $(
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        ...
        $itemEl = $(
          ...
          `<a class="item" href="javascript:;"  data-abbr="${abbr}"><span></span></a>`
        );
    ...
    SimditorMention.prototype._changeFocus = function (type) {
      ...
      if (position.top > parentH - itemH) {
        parentEl.scrollTop(
          itemH * itemEl.prevAll(".item:visible").length - parentH + itemH
        );
      }
      if (position.top < 0) {
        return parentEl.scrollTop(
          itemH * itemEl.prevAll(".item:visible").length
        );
      }
    };
    ...
    SimditorMention.prototype._onKeyUp = function (e) {
      ...
      if (range.toString() === "@" && !this.active) {
        ...
        range.setStart(
          ...
          Math.max(range.startOffset - 2, 0)
        );
    ...
    SimditorMention.prototype.refresh = function () {
      ...
      if (
        targetOffset.top - $(document).scrollTop() + popoverH >
        $(window).height()
      ) {
    ...
    SimditorMention.prototype.filterItem = function () {
      ...
      results = $itemEls
        ...
        .filter(function (i) {
          var $el, str;
          $el = $(this);
          str = [$el.data("name"), $el.data("abbr")].join(" ");
          return re.test(str);
        });
      if (results.length) {
        this.popoverEl.show();
